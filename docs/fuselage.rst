...
| :math:`N_{land}` & :math:`~[-]` & Emergency landing load factor
| :math:`N_{lift}` & :math:`~[-]` & Wing maximum load factor
| :math:`P_{cabin}` & :math:`~\mathrm{\tfrac{N}{m^{2}}}` & Cabin air
  pressure
| :math:`R` & :math:`~\mathrm{\tfrac{J}{kgK}}` & Air specific heat
| :math:`SPR` & :math:`~[-]` & Number of seats per row
| :math:`T_{cabin}` & :math:`~[K]` & Cabin air temperature
| :math:`W''_{floor}` & :math:`~\mathrm{\tfrac{N}{m^{2}}}` & Floor
  weight/area density
| :math:`W''_{insul}` & :math:`~\mathrm{\tfrac{N}{m^{2}}}` & Weight/area
  density of insulation material
| :math:`W'_{seat}` & :math:`~\mathrm{N}` & Weight per seat
| :math:`W'_{window}` & :math:`~\mathrm{\tfrac{N}{m}}` & Weight/length
  density of windows
| :math:`W_{avg. pass_{total}}` & :math:`~\mathrm{lbf}` & Average
  passenger weight including payload
| :math:`W_{avg. pass}` & :math:`~\mathrm{lbf}` & Average passenger
  weight
| :math:`W_{cargo}` & :math:`~\mathrm{lbf}` & Cargo weight
| :math:`W_{carry on}` & :math:`~\mathrm{lbf}` & Ave. carry-on weight
| :math:`W_{checked}` & :math:`~\mathrm{lbf}` & Ave. checked bag weight
| :math:`W_{fix}` & :math:`~\mathrm{lbf}` & Fixed weights (pilots,
  cockpit seats, navcom)
| :math:`\Delta P_{over}` & :math:`~\mathrm{psi}` & Cabin overpressure
| :math:`\Delta R_{fuse}` & :math:`~\mathrm{m}` & Fuselage extension
  height
| :math:`\lambda_{cone}` & :math:` ` [-]:math:` ` & Tailcone radius
  taper ratio
| :math:`\rho_{bend}` & :math:`~\mathrm{\tfrac{kg}{m^{3}}}` & Stringer
  density
| :math:`\rho_{cargo}` & :math:`~\mathrm{\tfrac{kg}{m^{3}}}` & Cargo
  density
| :math:`\rho_{cone}` & :math:`~\mathrm{\tfrac{kg}{m^{3}}}` & Cone
  material density
| :math:`\rho_{floor}` & :math:`~\mathrm{\tfrac{kg}{m^{3}}}` & Floor
  material density
| :math:`\rho_{lugg}` & :math:`~\mathrm{\tfrac{kg}{m^{3}}}` & Luggage
  density
| :math:`\rho_{skin}` & :math:`~\mathrm{\tfrac{kg}{m^{3}}}` & Skin
  density
| :math:`\sigma_{bend}` & :math:`~\mathrm{\tfrac{N}{m^{2}}}` & Bending
  material stress
| :math:`\sigma_{floor}` & :math:`~\mathrm{\tfrac{N}{m^{2}}}` & Max
  allowable floor stress
| :math:`\sigma_{skin}` & :math:`~\mathrm{\tfrac{N}{m^{2}}}` & Max
  allowable skin stress
| :math:`\tau_{floor}` & :math:`~\mathrm{\tfrac{N}{m^{2}}}` & Max
  allowable shear web stress
| :math:`f_{apu}` & :math:`~[-]` & APU weight as fraction of payload
  weight
| :math:`f_{fadd}` & :math:`~[-]` & Fractional added weight of local
  reinforcements
| :math:`f_{frame}` & :math:`~[-]` & Fractional frame weight
| :math:`f_{lugg,1}` & :math:`~[-]` & Proportion of passengers with one
  suitcase
| :math:`f_{lugg,2}` & :math:`~[-]` & Proportion of passengers with two
  suitcases
| :math:`f_{padd}` & :math:`~[-]` & Other misc weight as fraction of
  payload weight
| :math:`f_{seat}` & :math:`~[-]` & Fractional seat weight
| :math:`f_{string}` & :math:`~[-]` & Fractional stringer weight
| :math:`g` & :math:`~\mathrm{\tfrac{m}{s^{2}}}` & Acceleration due to
  gravity
| :math:`h_{floor}` & :math:`~\mathrm{m}` & Floor beam height
| :math:`l_{nose}` & :math:`~\mathrm{m}` & Nose length
| :math:`n_{pass}` & :math:`~[-]` & Number of passengers
| :math:`p_s` & :math:`~\mathrm{cm}` & Seat pitch
| :math:`p_{\lambda_v}` & :math:`~[-]` & 1 + 2\*Tail taper ratio
| :math:`r_E` & :math:`~[-]` & Ratio of stringer/skin moduli
| :math:`r_{M_h}` & :math:`~[-]` & Horizontal inertial relief factor
| :math:`r_{M_v}` & :math:`~[-]` & Vertical inertial relief factor
| :math:`r_{w/c}` & :math:`~[-]` & Wingbox width-to-chord ratio
| :math:`w_{aisle}` & :math:`~\mathrm{m}` & Aisle width
| :math:`w_{seat}` & :math:`~\mathrm{m}` & Seat width
| :math:`w_{sys}` & :math:`~\mathrm{m}` & Width between cabin and skin
  for systems
