...
case :raw-latex:`\cite{raymer1992aircraft}`. The vertical tail must also
...
structure model from :raw-latex:`\cite{hoburg2014geometric}` is also
...
inoperative :raw-latex:`\cite{drela2010tasopt}`.
...
theory :raw-latex:`\cite{andersonfundamentals}`.
...
area :raw-latex:`\cite{drela2010tasopt}`, in this case the area of the
engine fan.
...
XFOIL:raw-latex:`\cite{drela1989xfoil}` data for the symmetric NACA 0008
through 0020 airfoils. The fit considers airfoil thickness, Mach number,
and Reynolds number. It was developed with
GPfit:raw-latex:`\cite{gpfitpaper,gpfit}` and has an RMS error of 1.31%.
